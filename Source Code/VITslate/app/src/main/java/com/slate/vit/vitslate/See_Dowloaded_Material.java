...
public class See_Dowloaded_Material extends ActionBarActivity implements AdapterView.OnItemClickListener
{
    ...
    String subject_title = " ", faculty_title = " ";
    ListView lv;
    List<String> items;
    String dummyReverse = " ";
    int fileType;
    List<RowItemDownloaded> rowItems = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_material__downloaded);
        ...
        faculty_title = getIntent().getStringExtra("faculty_title");

        mDrawerList = (ListView)findViewById(R.id.navList);
        mDrawerLayout = (DrawerLayout)findViewById(R.id.drawer_layout);
        mActivityTitle = getTitle().toString();

        getSupportActionBar().setTitle(faculty_title);
        addDrawerItems();
        setupDrawer();
        ...
        lv = (ListView) findViewById(R.id.list_MD);
        items = new ArrayList<String>();

        File appDirectory = new File(rootDirectory + "/VITSlate/WinSem2015/"+subject_title+"/"+faculty_title);

        if (!appDirectory.exists()) {
            appDirectory.mkdirs();
        }

        File f = new File(appDirectory.toString());

        File[] files = f.listFiles();
        for (File inFile : files) {
            items.add(inFile.getName());
            }


        for (int i = 0; i < items.size(); i++)
        {
            dummyReverse = new StringBuffer(items.get(i).toString()).reverse().toString().substring(0,3);

            Log.d("Debug",dummyReverse.substring(0,3));

            switch(dummyReverse)
            {
                case "tpp":
                    fileType = R.drawable.ppt;
                    break;
                case "xtp":
                    fileType = R.drawable.ppt;
                    break;
                case "cod":
                    fileType = R.drawable.doc;
                    break;
                case "xco":
                    fileType = R.drawable.doc;
                    break;
                case "fdp":
                    fileType = R.drawable.pdf;
                    break;
                case "txt":
                    fileType = R.drawable.txt;
                    break;
                default:
                    fileType = (R.drawable.file_type_unknown);
            }

            RowItemDownloaded item = new RowItemDownloaded( fileType , items.get(i).toString());
            rowItems.add(item);
        }

        lv = (ListView) findViewById(R.id.list_MD);
        MyAdapterDownload adapter = new MyAdapterDownload(this,
                R.layout.list_card_downloaded, rowItems);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position,
        ...
        File appDirectory = new File(rootDirectory + "/VITSlate/WinSem2015/"
                +subject_title+"/"+faculty_title+"/"+rowItems.get(position).toString());

        MimeTypeMap myMime = MimeTypeMap.getSingleton();
        Intent newIntent = new Intent(Intent.ACTION_VIEW);
        String mimeType = myMime.getMimeTypeFromExtension(fileExt(appDirectory.toString()).substring(1));
        newIntent.setDataAndType(Uri.fromFile(appDirectory), mimeType);
        newIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);

        try
        {
            this.getApplicationContext().startActivity(newIntent);
        }
        catch (ActivityNotFoundException e)
        {
            Toast toast = Toast.makeText(this.getApplicationContext(), "Error opening file.", Toast.LENGTH_LONG);
        ...
        try {
            ...
            class asynctask extends AsyncTask<Void, Void, String> {
                ...
                protected void onPreExecute() {
                    super.onPreExecute();
                    //Progress Bar
                    progress = new ProgressDialog(See_Dowloaded_Material.this, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT);
    ...
    public void captchaBuilder()
    {
        ...
        AlertDialog.Builder captchaBuilder = new AlertDialog.Builder(See_Dowloaded_Material.this, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT);
        LayoutInflater factory = LayoutInflater.from(See_Dowloaded_Material.this);
        ...
        class asynctask5 extends AsyncTask<Void, Void , String> {
            ...
            protected String doInBackground(Void... params) {
                ...
                catch(Exception e)
                {
                    ...
                    e.printStackTrace();
                    Intent iHome = new Intent(getApplicationContext(), See_Dowloaded_Material.class);
                    iHome.putExtra("subject_title", subject_title);
                    iHome.putExtra("faculty_title", faculty_title);
                    startActivity(iHome);
    ...
    void setThemeForApp()
    {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(See_Dowloaded_Material.this);
    ...
    void teamInfo()
    {
        ...
        Intent i2 = new Intent(See_Dowloaded_Material.this, Team.class);
        startActivity(i2);
...

    {
        ...
        Intent i2 = new Intent(See_Dowloaded_Material.this,Material_Downloaded.class);
